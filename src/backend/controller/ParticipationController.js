...
router.post('/', JWTService.requireJWT(), async (req, res) => {
  try {
    ...
    if (participation === null) {
      res.status(404).json({message: `Event with id "${eventId}" or User with with id "${userId}" does not exist or User is already participant.`});
    }
...
router.get('/:id', JWTService.requireJWT(), AuthService.compareId, async (req, res) => {
  try {
